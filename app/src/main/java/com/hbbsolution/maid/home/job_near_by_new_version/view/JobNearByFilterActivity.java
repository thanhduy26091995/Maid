...
public class JobNearByFilterActivity extends BaseActivity implements View.OnClickListener, OnClickDialogListener, ListJobView {
    ...
    private String mAddress, mTypeJobId, mTypeJobName;
    ...
    private void initData() {
        if (FilterModelSingleton.getInstance().getIsSaved()) {
            mFilterModel = FilterModelSingleton.getInstance().getFilterModel();
            //get data
            mLat = mFilterModel.getLat();
            mLng = mFilterModel.getLng();
            mAddress = mFilterModel.getAddressName();
            if (!TextUtils.isEmpty(mFilterModel.getAddressName())) {
                mTextViewLocation.setText(mFilterModel.getAddressName());
            } else {
                mTextViewLocation.setText(getResources().getString(R.string.near_by_filter_current_location));
            }
            mDistance = mFilterModel.getDistance();
            if (mDistance != null) {
                mTextViewDistance.setText(String.format("%d %s", mDistance, "km"));
            } else {
                mTextViewDistance.setText("5 km");
            }
            mTypeJobId = mFilterModel.getWorkId();
            if (mFilterModel.getWorkName() != null) {
                mTextViewTypeJob.setText(mFilterModel.getWorkName());
            } else {
                mTextViewTypeJob.setText(getResources().getString(R.string.near_by_filter_type_job_all));
            }
        }
    }
    ...
    public void onClick(View v) {
        if (v == mRelativeDistance) {
            ...
            Log.d("RESULT", "" + mLat + "/ " + mLng + "/ " + mAddress + "/ " + mTypeJobId + "/ " + mTypeJobName + "/ " + mDistance);
